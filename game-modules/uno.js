// game-modules/uno.js -- Server logic for "Cascade" (an original shedding-card
...
// hand is private (sent via "uno:hand"); the public "uno:state" only carries
...
function sendHands(api, s) {
  s.players.forEach((id) => api.toPlayer(id, "uno:hand", { hand: s.hands[id] }));
}
...
module.exports = {
  ...
  onCheat(api, room, cheatId) {
    ...
    if (cheatId === "reveal-hands") {
      ...
      if (s.hands) {
        ...
        api.toHost("uno:cheatReveal", { hands });
      }
      return;
    }
    if (s.phase !== "playing") return;
  ...
  onReconnect(api, room, who) {
    ...
    if (who && who.seatId && s.hands && s.hands[who.seatId]) {
      api.toPlayer(who.seatId, "uno:hand", { hand: s.hands[who.seatId] });
    }
    broadcast(api, room);
  },
