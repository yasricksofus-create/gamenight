...
(function () {
  ...
  // The long MUSIC tracks are mutually exclusive: only one plays at a time.
  // Starting one fades the others out (clean hand-off). Short sound effects
  // (distribution, vote, elimination, mrwhite) play OVER the current music.
  const MUSIC = ["reglages", "ambiance", "victoire"];
  function playMusic(key) {
    MUSIC.forEach((k) => { if (k !== key) Sound.fadeOut(k, 500); });
    Sound.play(key);
  }
  ...
  function phaseSound(next, s) {
    ...
    if (next === "settings") playMusic("reglages");
    else if (next === "distribution") { Sound.play("distribution"); playMusic("ambiance"); }
    ...
    else if (next === "ended") playMusic("victoire");
  }
